#!/usr/bin/env python3
"""
Standalone base motor test for ESP32 (encoder + TB6612).

  cd backend && python tests/test_base_motor.py --watch
  python tests/test_base_motor.py --calibrate-manual --degrees 90
  python tests/test_base_motor.py --relative 30 --verify
  python tests/test_base_motor.py --apply-config-cpd --relative 30 --verify
"""
...
def poll_encoder_loop(link: ArduinoServoLink, stop: threading.Event) -> None:
    ...
    if max_abs < MIN_ENCODER_DELTA:
        print(
            ...
            "  3. Reflash firmware: arduino-cli upload -p /dev/ttyUSB0 "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Test base motor via ESP32 USB serial")
