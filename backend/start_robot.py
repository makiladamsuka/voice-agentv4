#!/usr/bin/env python3
"""
Face Tracking Eyes for Dual SPI Displays (USB webcam or Picamera2).
Combines face tracking (YuNet) with dual SPI display output (ST7735).

Optional: ESP32 USB (arduino backend) or Pi ServoKit (legacy) pan/tilt — head held at center.

Launcher: python start_robot.py  (voice agent + eyes)
Eyes only: python start_robot.py --eyes-worker
"""
...
def start_services() -> None:
    ...
    eyes_proc = subprocess.Popen([sys.executable, __file__, "--eyes-worker"])
    ...
    try:
        ...
        print("🛑 Stopping services (eyes worker homes servos & blanks TFTs)...")
        for proc, name in ((eyes_proc, "eyes"), (voice_proc, "voice")):
            if proc.poll() is None:
                proc.send_signal(signal.SIGINT)
        deadline = time.time() + 8.0
        while time.time() < deadline:
            if eyes_proc.poll() is not None and voice_proc.poll() is not None:
                break
            time.sleep(0.1)
        for proc, name in ((eyes_proc, "eyes"), (voice_proc, "voice")):
            if proc.poll() is None:
                print(f"   {name} still running — terminating...")
                proc.terminate()
        eyes_proc.wait(timeout=3.0)
        voice_proc.wait(timeout=3.0)
        print("✅ Shutdown complete.")


if __name__ == "__main__" and "--eyes-worker" not in sys.argv:
    start_services()
    sys.exit(0)
...
DETECT_UPSCALE = max(1.0, float(_c.detect_upscale))
DETECT_INPUT_SIZE = (
    int(CAMERA_RES[0] * DETECT_UPSCALE),
    int(CAMERA_RES[1] * DETECT_UPSCALE),
)
...
NO_FACE_TILT_MAX = _ft.no_face_tilt_max
NO_FACE_TILT_MIN = _ft.no_face_tilt_min
...
TOF_NO_TARGET_MM = int(_tof.no_target_mm)
...
def sync_config_from_cfg() -> None:
    ...
    global WANDER_SEARCH_TILT_PHASE_K, NO_FACE_TILT_MAX, NO_FACE_TILT_MIN
    ...
    global DETECT_UPSCALE, DETECT_INPUT_SIZE
...
def handle_api_trigger(data: dict) -> dict:
    ...
    global wake_request_ts, amplitude_fast, amplitude_slow, udp_speak_pulse, animation_arm_targets
...
latest_frame = None
last_stream_box = None  # last YuNet bbox on debug stream (stream coords)
...
class MJPEGHandler(BaseHTTPRequestHandler):
    ...
    def _serve_asset(self):
        if not self.path.startswith("/assets/"):
            self.send_error(404)
            return
        file_path = _ASSETS_DIR.parent / self.path.lstrip("/")
        try:
            file_path.resolve().relative_to(_ASSETS_DIR.parent.resolve())
        except ValueError:
            self.send_error(403)
            return
        ...
        content_type = _ASSET_CONTENT_TYPES.get(file_path.suffix.lower(), "application/octet-stream")
        self.send_response(200)
        self.send_header("Content-Type", content_type)
        self.send_header("Content-Length", str(len(data)))
...
print(f"Initializing camera (backend={_c.backend!r}, device={_c.device_index})...")
camera = None
try:
    camera = open_camera(_c)
    print(
        f"Camera started: main {CAMERA_MAIN_RES[0]}x{CAMERA_MAIN_RES[1]}, "
        f"detect {CAMERA_RES[0]}x{CAMERA_RES[1]}"
        + (
            f" (YuNet @ {DETECT_INPUT_SIZE[0]}x{DETECT_INPUT_SIZE[1]}, "
            f"upscale {DETECT_UPSCALE:.2f}x)"
            if DETECT_UPSCALE > 1.0
            else ""
        )
    )
except Exception as e:
...
filtered_norm_x = 0.0
filtered_norm_y = 0.0
servo_filtered_pan = _pan_center_init
servo_filtered_tilt = _tilt_center_init
...
def head_center_angles() -> tuple[float, float]:
    """Fixed head pose — pan/tilt center of configured range."""
    return (PAN_MIN + PAN_MAX) * 0.5, (TILT_MIN + TILT_MAX) * 0.5


def clamp_no_face_tilt(angle: float) -> float:
    """Keep no-face tilt in a forward band — avoid looking straight up."""
    return clamp(angle, NO_FACE_TILT_MIN, NO_FACE_TILT_MAX)


def no_face_tilt_neutral() -> float:
    """Forward tilt for idle/wander, capped below skyward angles on this rig."""
    return clamp_no_face_tilt((TILT_MIN + TILT_MAX) * 0.5)
...
def servo_worker():
    ...
    while servo_running:
        ...
        conv_tilt = 0.0
        if not face_locked and no_face_mode == "chat_ready":
        ...
        try:
            ...
            frame_tokens["A0="] = clamp(arm_targets.get("arm_0", neutral_arm_targets.get("arm_0", 0.0)), 0.0, 180.0)
            frame_tokens["A1="] = clamp(arm_targets.get("arm_1", neutral_arm_targets.get("arm_1", 180.0)), 0.0, 180.0)
            frame_tokens["A2="] = clamp(arm_targets.get("arm_2", neutral_arm_targets.get("arm_2", 90.0)), 0.0, 180.0)
            frame_tokens["A3="] = clamp(arm_targets.get("arm_3", neutral_arm_targets.get("arm_3", 90.0)), 0.0, 180.0)
...
def get_runtime_state() -> dict:
    ...
    with servo_state_lock:
        ...
        servo = {
            ...
            "pan_ch": PAN_CH,
            "tilt_ch": TILT_CH,
            "pan_axis": "PCA ch4 / serial P",
            "tilt_axis": "PCA ch5 / serial T",
            "animation_clip": active_clip,
        }
...
def scale_yunet_face(face_row, inv_scale: float):
    """Map YuNet box from upscaled detect coords back to detect_res space."""
    if inv_scale == 1.0:
        return face_row
    out = list(face_row)
    for i in range(8):
        out[i] = float(out[i]) * inv_scale
    return out


def prepare_frames_for_detection(large_rgb: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Build BGR frames for YuNet (BGR) and RGB preview (stream/eyes).
    Returns (detect_frame_bgr, preview_rgb_unrotated, inv_upscale).
    """
    frame_bgr = cv2.cvtColor(large_rgb, cv2.COLOR_RGB2BGR)
    h, w = frame_bgr.shape[:2]
    if (w, h) != (CAMERA_RES[0], CAMERA_RES[1]):
    ...
    if DETECT_UPSCALE > 1.0:
        ...
        inv_upscale = 1.0 / DETECT_UPSCALE
    detect_frame = (
        cv2.rotate(detect_bgr, cv2.ROTATE_180) if CAMERA_ROTATE_180 else detect_bgr
    )
    preview_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
    return detect_frame, preview_rgb, inv_upscale
...
def draw_face_debug_on_stream(preview_bgr, box, *, primary: bool, label: str = "") -> None:
    """Draw YuNet face box on BGR preview (debug /stream)."""
    if box is None:
        return
    fx_s, fy_s, fw_s, fh_s, re_x_s, re_y_s, le_x_s, le_y_s = box
    color = (0, 255, 0) if primary else (255, 220, 0)
    thickness = 3 if primary else 1
    cv2.rectangle(
        preview_bgr,
        (fx_s, fy_s),
        (fx_s + fw_s, fy_s + fh_s),
    ...
    cv2.circle(preview_bgr, (re_x_s, re_y_s), 4, (255, 128, 0), -1)
    cv2.circle(preview_bgr, (le_x_s, le_y_s), 4, (255, 128, 0), -1)
    if label:
        cv2.putText(
            preview_bgr,
            label,
...
def vision_worker():
    ...
    global wander_search_phase, last_stream_box
    ...
    while running:
        try:
            large_rgb = camera.capture_rgb()
            frame, preview_rgb, det_inv_scale = prepare_frames_for_detection(large_rgb)
            ...
            if frame is not None and frame.size > 0:
                stream_bgr = None
                if STREAM_ENABLED:
                    stream_rgb = cv2.resize(preview_rgb, STREAM_RES)
                    stream_bgr = cv2.cvtColor(stream_rgb, cv2.COLOR_RGB2BGR)
                ...
                if faces[1] is not None:
                    has_face = True
                    detected_faces = [
                        scale_yunet_face(f, det_inv_scale) for f in faces[1]
                    ]
                    ...
                    if STREAM_ENABLED and stream_bgr is not None:
                        for f in detected_faces:
                            stream_boxes.append(
                                map_coords_to_stream_preview(
                                    float(f[0]), float(f[1]), float(f[2]), float(f[3]),
                                    float(f[4]), float(f[5]), float(f[6]), float(f[7]),
                                )
                            )
                        stream_box = map_coords_to_stream_preview(
                            fx, fy, fw, fh, re_x, re_y, le_x, le_y,
                        )
                        last_stream_box = stream_box
                ...
                if ENABLE_SERVO and servo_driver is not None and not face_locked:
                    ...
                    tilt_neutral = no_face_tilt_neutral()
                    tilt_idle = clamp_no_face_tilt(
                        tilt_neutral + NO_FACE_IDLE_TILT_DEG
                    )
                    with servo_state_lock:
                        if no_face_mode == "wandering":
                            ...
                            servo_target_tilt = clamp_no_face_tilt(
                                tilt_neutral + tilt_wiggle
                            )
                        elif no_face_mode == "sad_return":
                            ...
                            servo_target_tilt = clamp_no_face_tilt(
                                servo_target_tilt
                                + (tilt_idle - servo_target_tilt) * NO_FACE_SAD_RECENTER_ALPHA
                            )
                        elif no_face_mode == "chat_ready":
                            servo_target_pan = servo_target_pan + (
                            ...
                            servo_target_tilt = servo_target_tilt + (
                                tilt_neutral - servo_target_tilt
                            ) * CHAT_READY_RECENTER_ALPHA
                            servo_target_tilt = clamp_no_face_tilt(servo_target_tilt)
                            ...
                            if now_vis - last_face_seen_ts > NO_FACE_RECENTER_SEC:
                                ...
                                servo_target_tilt = clamp_no_face_tilt(
                                    servo_target_tilt
                                    + (tilt_idle - servo_target_tilt) * NO_FACE_RECENTER_ALPHA
                                )
                ...
                with target_lock:
                    ...
                    face_tracking_active = face_locked

                if STREAM_ENABLED and stream_bgr is not None:
                    preview_bgr = stream_bgr.copy()
                    status = "TRACK" if face_locked else "NO FACE"
                    status_color = (80, 255, 80) if face_locked else (80, 80, 255)
                    cv2.putText(
                        preview_bgr,
                        status,
                        (8, 18),
                    ...
                    draw_box = stream_box if stream_box is not None else (
                        last_stream_box if face_locked else None
                    )
                    if has_face and stream_boxes:
                        primary = draw_box
                        for sb in stream_boxes:
                            draw_face_debug_on_stream(
                                preview_bgr,
                                sb,
                                primary=(primary is not None and sb == primary),
                                label="FACE" if primary is not None and sb == primary else "",
                            )
                    elif draw_box is not None:
                        draw_face_debug_on_stream(
                            preview_bgr, draw_box, primary=True, label="FACE"
                        )
                    with servo_state_lock:
                        dbg_pan = servo_current_pan
                        dbg_tilt = servo_current_tilt
                        dbg_tgt_pan = servo_target_pan
                        dbg_tgt_tilt = servo_target_tilt
                    cv2.putText(
                        preview_bgr,
                        f"HEAD P={dbg_pan:.0f} T={dbg_tilt:.0f} tgt {dbg_tgt_pan:.0f}/{dbg_tgt_tilt:.0f}",
                        (8, 36),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.38,
                        (180, 220, 255),
                    ...
                    if face_locked:
                        cv2.putText(
                            preview_bgr,
                            f"nx={last_face_norm_x:+.2f} ny={last_face_norm_y:+.2f}",
                            (8, 52),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.42,
                            (200, 255, 200),
                    ...
                    if TOF_ENABLED:
                        ...
                        cv2.putText(
                            preview_bgr,
                            f"ToF L={ts.left_mm} C={ts.center_mm} R={ts.right_mm} "
                            f"pres {int(tp.left)}{int(tp.center)}{int(tp.right)}",
                            (8, 68),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.36,
                            (180, 200, 255),
                    ...
                    with frame_lock:
                        latest_frame = cv2.cvtColor(preview_bgr, cv2.COLOR_BGR2RGB)
...
_loaded_clips = _load_default_animation_clips()
if _loaded_clips:
    print(f"Loaded {_loaded_clips} animation clip(s) from backend/animations")
...
def safe_home_servos() -> None:
    """Smoothly move pan/tilt to configured center before power-down."""
    ...
    with servo_state_lock:
        pan = servo_current_pan
        tilt = servo_current_tilt
    alpha = max(SMOOTHING, 0.06)
    print(f"Homing servos to P={pan_home:.1f} T={tilt_home:.1f}...")
    for _ in range(150):
        pan += (pan_home - pan) * alpha
        tilt += (tilt_home - tilt) * alpha
        try:
            servo_driver.write_angles(pan, tilt, force=True)
        except Exception as e:
            print(f"Servo homing step: {e}")
            break
        if abs(pan - pan_home) < 0.3 and abs(tilt - tilt_home) < 0.3:
            break
        time.sleep(SERVO_LOOP_DELAY)
    try:
        servo_driver.write_angles(pan_home, tilt_home, force=True)
    except Exception as e:
        print(f"Servo final home: {e}")
    time.sleep(0.12)
    with servo_state_lock:
        servo_current_pan = pan_home
        servo_current_tilt = tilt_home
        servo_target_pan = pan_home
        servo_target_tilt = tilt_home


def shutdown_displays() -> None:
    """Blank TFTs and release display handles."""
    global disp_l, disp_r
    black = Image.new("RGB", (SCREEN_WIDTH, SCREEN_HEIGHT), (0, 0, 0))
    for label, disp in (("left", disp_l), ("right", disp_r)):
        if disp is None:
            continue
        try:
            disp.image(black)
        except Exception as e:
            print(f"TFT {label} clear: {e}")
    time.sleep(0.06)
    for label, disp in (("left", disp_l), ("right", disp_r)):
        if disp is None:
            continue
        try:
            deinit = getattr(disp, "deinit", None)
            if callable(deinit):
                deinit()
        except Exception as e:
            print(f"TFT {label} deinit: {e}")
    disp_l = None
    disp_r = None
    print("TFT displays off.")


def shutdown_robot() -> None:
    """Ctrl+C / SIGTERM: stop threads, home servos, blank screens, close devices."""
    global _shutdown_done, running, servo_running
    if _shutdown_done:
        return
    _shutdown_done = True
    ...
    if servo_driver is not None:
        ...
        try:
            ...
            print("Head servos at default pose, serial closed.")
        except Exception as e:
            print(f"Servo driver close: {e}")
    try:
        if camera:
            camera.close()
            print("Camera closed.")
